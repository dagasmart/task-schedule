...
class TaskScheduleGroupController extends AdminController
{
		...
		$crud = $this->baseCRUD()
            ...
            ->filter(
                $this->baseFilter()->body([
                    amis()->TextControl('group_name', '分组名称')->clearable()->size('sm'),
                    amis()->Divider(),
		...
		return $this->baseForm()->mode('normal')->body([
            amis()->GroupControl()->direction('vertical')->body([
                ...
                amis()->CheckboxesControl('module', '模块')
                    ...
                    ->options($this->service->moduleOption()),
                amis()->NumberControl('sort', '排序[0-255]')->value(10)->size('sm'),
            ]),
		]);
	}
	...
	public function detail(): Form
    {
		return $this->baseDetail()->body([
			...
			amis()->TextControl('group_name', '分组名称')->static(),
            amis()->TextareaControl('description', '分组描述')->static(),
            amis()->CheckboxesControl('module', '模块')
                ->options($this->service->moduleOption())
                ->disabled(),
