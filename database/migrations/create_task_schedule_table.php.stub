...
class CreateTaskScheduleTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(Config::get('schedule.table', 'task_schedule'), function (Blueprint $table) {
            $table->comment('任务调度表');
            $table->id();
            $table->string('task_name', 64)->nullable()->comment('任务名称');
            ...
            $table->string('expression', 100)->nullable()->comment('执行时间间隔，cron 表达式');
            $table->boolean('active')->default(false)->comment('运行状态：0 开启，1 关闭');
            $table->string('timezone', 32)->comment('时区');
            ...
            $table->string('output_email', 100)->nullable()->comment('输出发送邮件');
            ...
            $table->index('active');
            $table->index('command');
            $table->unique(['task_name', 'command', 'module', 'mer_id'])->nullsNotDistinct();
        });
    }
